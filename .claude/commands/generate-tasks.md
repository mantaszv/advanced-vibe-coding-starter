...
3. **Guards** — Specify which guardrails to run PRE (before implementation), POST (after), or both:
   - `payment-guardian` — payment code changes
   - `db-guardian` — database migrations
   - `lang-reviewer` — UI text in correct language
   - `pre-deploy` — before commit
   - `file-splitter` — splitting large files
...
FINAL VERIFICATION:
+-- Task 5: {description} [Guard: pre-deploy, lang-reviewer]
```
