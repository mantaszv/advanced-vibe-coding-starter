...
BATCH VERIFY:
  ...
  2. Execute Verify steps:
     - (nesukonfigūruota) → compilation
     - (nesukonfigūruota) → 0 errors, 0 warnings
     - bash scripts/verify.sh → all tests pass
...
- Run `bash scripts/verify.sh` after batch completion
