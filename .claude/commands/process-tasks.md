...
1. **PRE Guards** — Run all PRE guards before any work:
   - `payment-guardian` → if touching payment code
   - `db-guardian` → if DB migration planned
   - `file-splitter` → if splitting a large file
...
5. **POST Guards** — After implementation:
   - `lang-reviewer` → check UI text language
   - `db-guardian` → verify migration
...
- **Run tests after each sub-task:** `npm run test:run`
- **Minimum coverage: 80%**
